...
def mock_context_processing(monkeypatch):
    monkeypatch.setattr("src.main.process_stock", lambda: None)
...
def test_analytics_sales(client, monkeypatch, success_code):
    monkeypatch.setattr("src.main.process_stock", lambda: None)
...
def test_analytics_sales_db_disabled(client, monkeypatch, success_code):
...
def test_analytics_sales_db_fetching_sales_failed(client, monkeypatch, success_code):
    def _mock_fetch_sales_fething():
        raise RuntimeError("Failed to fetch sales data")
    monkeypatch.setattr("src.main.fetch_sales_from_db",
                        _mock_fetch_sales_fething)
    response = client.get("/analytics/sales")
    assert response.status_code == success_code
    assert response.json() == {"sales": "sales_data_not_available"}


def test_analytics_stock(client, monkeypatch, success_code):
    monkeypatch.setattr("src.main.process_stock", lambda: None)
